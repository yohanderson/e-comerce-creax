...
function loadProductsCart() {
    if (productsInCart && productsInCart.length > 0) {
        ...
        productsInCart.forEach(product => {
            ...
            div.innerHTML = `
                <img class="cart-product-image" src="${product.image}" alt="${product.titule}">
...
function emptyCart() {
    ...
    Swal.fire({
        title: '¿Estás seguro?',
        icon: 'question',
        html: `Se van a borrar ${productsInCart.reduce((acc, product) => acc + product.amount, 0)} products.`,
...
function updateTotal() {
    const totalCalculator = productsInCart.reduce((acc, product) => acc + (product.price * product.amount), 0);
